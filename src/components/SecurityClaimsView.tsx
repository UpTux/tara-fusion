...
import { Project, SecurityClaim, Assumption } from '../types';
    ...
    <div className="flex h-full text-white">
      <div className="w-2/5 border-r border-gray-700/50 flex flex-col">
        <div className="p-4 border-b border-gray-700/50 flex justify-between items-center">
          <h2 className="text-lg font-semibold">Security Claims</h2>
          <button onClick={handleAdd} title="Add new security claim" className="p-1.5 text-gray-400 hover:text-white hover:bg-gray-700 rounded disabled:opacity-50 disabled:cursor-not-allowed" disabled={isReadOnly}><PlusIcon className="w-5 h-5" /></button>
        </div>
        <div className="flex-1 overflow-y-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {claims.map(claim => (
                <tr key={claim.id} onClick={() => setSelectedId(claim.id)} className={`border-t border-gray-700/50 cursor-pointer transition-colors ${selectedId === claim.id ? 'bg-indigo-600/20' : 'hover:bg-gray-800/50'}`}>
      ...
      <div className="w-3/5 flex-1 overflow-y-auto p-8">
        {editorState ? (
          ...
          <div className="flex items-center justify-center h-full text-gray-500"><div className="text-center"><h3 className="text-lg">No Security Claim Selected</h3><p>Select a claim from the list or create a new one.</p></div></div>
        )}
      </div>
    </div>
  );
