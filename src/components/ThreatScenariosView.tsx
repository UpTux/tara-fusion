...
import {
  ...
  knowledgeOptions,
  timeOptions,
} from "../services/feasibilityOptions";
import {
  calculateAP,
...
const Label: React.FC<{
  ...
  <label
    htmlFor={htmlFor}
    className={`block text-sm font-medium text-vscode-text-secondary mb-1 ${className}`}
  >
    {children}
  </label>
);
const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = (
  ...
  <input
    {...props}
    className="block w-full px-3 py-2 bg-vscode-bg-input border border-vscode-border rounded-md shadow-sm placeholder-vscode-text-secondary focus:outline-none focus:ring-vscode-accent focus:border-vscode-accent sm:text-sm text-vscode-text-primary disabled:bg-vscode-bg-input/50 disabled:cursor-not-allowed"
  />
);
...
const MultiSelectDropdown: React.FC<{
  ...
  label: string;
  disabled?: boolean;
}> = ({ options, selected, onUpdate, label, disabled = false }) => {
      ...
      {isOpen && !disabled && (
        <div className="absolute z-10 w-full mt-1 bg-vscode-bg-sidebar border border-vscode-border rounded-md shadow-lg max-h-60 overflow-y-auto">
          {options.map((option) => (
            <label
              key={option.id}
              className="flex items-center px-3 py-2 text-sm text-vscode-text-primary hover:bg-vscode-accent/20 cursor-pointer"
            >
              <input
                type="checkbox"
                checked={selected.includes(option.id)}
                onChange={() => handleSelect(option.id)}
                className="h-4 w-4 rounded border-vscode-border bg-vscode-bg-input text-vscode-accent focus:ring-vscode-accent"
              />
              <span className="ml-3 truncate" title={option.name}>
                {option.name}
              </span>
            </label>
          ))}
...
export const ThreatScenariosView: React.FC<ThreatScenariosViewProps> = ({
  project,
  onUpdateProject,
  isReadOnly,
}) => {
  const [selectedId, setSelectedId] = useState<string | null>(
    () => (project.threatScenarios || [])[0]?.id || null
  );
  ...
  const lastSyncedRef = useRef<ThreatScenario | null>(editorState);

  useEffect(() => {
    const scenarios = project.threatScenarios || [];
    const selected = scenarios.find((ds) => ds.id === selectedId);
    if (
      JSON.stringify(selected ? { ...selected } : null) !==
      JSON.stringify(lastSyncedRef.current)
    ) {
      ...
      onConfirm: () => {
        ...
        const updatedThreats = (project.threats || []).map((t) => {
          if (t.id === scenarioToDelete.threatId) {
            return {
              ...t,
              damageScenarioIds: t.damageScenarioIds.filter(
                (dsId) => !scenarioToDelete.damageScenarioIds.includes(dsId)
              ),
            };
          }
          return t;
        ...
        const updatedProject = addHistoryEntry(
          {
            ...
            threatScenarios: updatedScenarios,
          },
          `Deleted Threat Scenario ${selectedId} and its threat link.`
        );
        onUpdateProject(updatedProject);
  ...
  const derivedImpact = useMemo(() => {
    ...
    return calculateHighestImpact(
      editorState.damageScenarioIds,
  ...
  const attackTreeMetrics = useMemo(() => {
    ...
    const attackRootNode = project.needs?.find(
      (need) =>
        need.type === "ATTACK" &&
    ...
    return calculateAttackTreeMetrics(
      attackRootNode.id,
      project.needs || [],
  ...
  const riskLevel = useMemo(() => {
    if (!editorState || !derivedImpact || !effectiveFeasibilityRating)
      return null;
  ...
  const scenariosWithRisk = useMemo(() => {
    return (project.threatScenarios || []).map((ts) => {
      const impact = calculateHighestImpact(
        ts.damageScenarioIds,
        project.damageScenarios || []
      );
      ...
      if (attackRootNode) {
        ...
        const treeMetrics = calculateAttackTreeMetrics(
          attackRootNode.id,
          project.needs || [],
          project.toeConfigurations
        );
    ...
    <div className="flex h-full text-vscode-text-primary">
      ...
      <div className="w-2/5 border-r border-vscode-border flex flex-col">
        <div className="p-4 border-b border-vscode-border flex justify-between items-center">
          <h2 className="text-2xl font-bold text-vscode-text-primary">
            Threat Scenarios
          </h2>
          {/* Add button removed */}
        </div>
        <div className="flex-1 overflow-y-auto">
          <table className="w-full text-sm text-left">
            <thead className="sticky top-0 bg-vscode-bg-sidebar backdrop-blur-sm">
              <tr>
                <th className="p-3 font-semibold tracking-wider text-vscode-text-primary text-vscode-text-primary">
                  ID
                </th>
                <th className="p-3 font-semibold tracking-wider text-vscode-text-primary text-vscode-text-primary">
                  Name
            ...
            <tbody>
              {scenariosWithRisk.map((ts) => (
                <tr
                  ...
                  className={`border-t border-vscode-border cursor-pointer transition-colors ${
                    selectedId === ts.id
                      ? "bg-vscode-accent/20"
                ...
                >
                  ...
                  <td className="p-3">
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-semibold text-white ${getRiskColor(
                        ts.risk
                      )}`}
                    >
                      {ts.risk}
        ...
        {editorState ? (
          <div className="space-y-8">
            <div className="flex justify-between items-start">
              <h2 className="text-2xl font-bold text-vscode-text-primary">
                {editorState.id}: {editorState.name}
              </h2>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditorState({ ...editorState, id: e.target.value })
                  }
                  disabled={isReadOnly}
                />
              </div>
              ...
              <div className="col-span-2">
                ...
                <p className="p-2 bg-vscode-bg-sidebar rounded-md text-vscode-text-primary">
                  {project.threats?.find((t) => t.id === editorState.threatId)
                    ?.name || editorState.threatId}
                </p>
              </div>
              <div className="col-span-2">
                <MultiSelectDropdown
                  label="Resulting Damage Scenarios"
                  options={(project.damageScenarios || []).map((ds) => ({
                    id: ds.id,
                    name: ds.name,
                  }))}
              ...
              <div className="col-span-2">
                <div className="flex items-center mb-1">
                  ...
                  <button
                    ...
                    className="ml-2 text-vscode-text-secondary hover:text-vscode-text-primary"
                    title="Open Attacker Capability Rating Guide"
                  >
                ...
                {attackTreeMetrics ? (
                  <div className="p-4 bg-vscode-bg-sidebar rounded-md border border-vscode-border">
                    <div className="text-center mb-3">
                      <div className="text-sm text-vscode-text-secondary mb-1">
                        Total Attack Potential
                      </div>
                      <div className="text-2xl font-bold font-mono text-indigo-300">
                        {attackTreeMetrics.attackPotential === Infinity
                          ? "∞"
                          : attackTreeMetrics.attackPotential}
                      </div>
                    ...
                    <div className="text-center">
                      <div className="text-xs text-vscode-text-secondary mb-2">
                        Individual values not available from attack tree
                        calculation
                      </div>
                      <div className="text-xs text-vscode-text-secondary">
                        The attack tree provides the optimal total attack
                        potential based on the critical path analysis.
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-3 xl:grid-cols-5 gap-4 p-4 bg-vscode-bg-sidebar rounded-md border border-vscode-border">
                    <div>
                      ...
                      <Select
                        id="feasibility-time"
                        value={editorState.attackPotential.time}
                        onChange={(e) =>
                    ...
                    <div>
                      <Label
                        htmlFor="feasibility-expertise"
                        className="capitalize text-xs"
                      >
                        Expertise
                      </Label>
                      <Select
                        id="feasibility-expertise"
                        value={editorState.attackPotential.expertise}
                        onChange={(e) =>
                          handleFeasibilityChange(
                            "expertise",
                            parseInt(e.target.value, 10)
                    ...
                    <div>
                      <Label
                        htmlFor="feasibility-knowledge"
                        className="capitalize text-xs"
                      >
                        Knowledge
                      </Label>
                      <Select
                        id="feasibility-knowledge"
                        value={editorState.attackPotential.knowledge}
                        onChange={(e) =>
                          handleFeasibilityChange(
                            "knowledge",
                            parseInt(e.target.value, 10)
                    ...
                    <div>
                      <Label
                        htmlFor="feasibility-access"
                        className="capitalize text-xs"
                      >
                        Access
                      </Label>
                      <Select
                        id="feasibility-access"
                        value={editorState.attackPotential.access}
                        onChange={(e) =>
                          handleFeasibilityChange(
                            "access",
                            parseInt(e.target.value, 10)
                    ...
                    <div>
                      <Label
                        htmlFor="feasibility-equipment"
                        className="capitalize text-xs"
                      >
                        Equipment
                      </Label>
                      <Select
                        id="feasibility-equipment"
                        value={editorState.attackPotential.equipment}
                        onChange={(e) =>
                          handleFeasibilityChange(
                            "equipment",
                            parseInt(e.target.value, 10)
              ...
              <div>
                ...
                <p className="p-2 bg-vscode-bg-sidebar rounded-md text-vscode-text-primary font-semibold">
                  {effectiveFeasibilityRating} (
                  {effectiveAttackPotential === Infinity
                    ? "∞"
                    : effectiveAttackPotential}
                  )
                </p>
              ...
              <div className="col-span-2">
                ...
                {riskLevel && (
                  <p
                    className={`px-3 py-1.5 rounded-md text-sm font-semibold text-white inline-block ${getRiskColor(
                      riskLevel
                    )}`}
                  >
                    {riskLevel}
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setEditorState({ ...editorState, comment: e.target.value })
                }
                disabled={isReadOnly}
              />
            </div>
          ...
          <div className="flex items-center justify-center h-full text-vscode-text-secondary">
            <div className="text-center">
              ...
              <p>
                Threat Scenarios are created automatically by linking Damage
                Scenarios to Threats in the 'Threats' view.
              </p>
            </div>
          </div>
        )}
