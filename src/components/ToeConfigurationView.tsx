    ...
    <div className="flex h-full text-white">
      ...
      <div className="w-1/3 border-r border-gray-700/50 flex flex-col">
        <div className="p-4 border-b border-gray-700/50 flex justify-between items-center">
          <h2 className="text-lg font-semibold">TOE Configurations</h2>
          <button onClick={handleAdd} title="Add new TOE configuration" className="p-1.5 text-gray-400 hover:text-white hover:bg-gray-700 rounded disabled:opacity-50 disabled:cursor-not-allowed" disabled={isReadOnly}>
            ...
            <tbody>
              {configurations.map(conf => (
                <tr
                  ...
                  className={`border-t border-gray-700/50 cursor-pointer transition-colors ${selectedId === conf.id ? 'bg-indigo-600/20' : 'hover:bg-gray-800/50'}`}
                >
                  <td className="p-3">
                    <span className={`w-3 h-3 rounded-full inline-block ${conf.active ? 'bg-green-500' : 'bg-gray-500'}`} title={conf.active ? 'Active' : 'Inactive'}></span>
                  </td>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex items-center pt-2">
                    <input id="confActive" type="checkbox" checked={editorState.active} onChange={e => handleUpdate('active', e.target.checked)} className="h-4 w-4 rounded border-gray-600 bg-gray-700 text-indigo-600 focus:ring-indigo-500 disabled:cursor-not-allowed" disabled={isReadOnly} />
                    <label htmlFor="confActive" className="ml-3 block text-sm font-medium text-gray-300">Active</label>
                </div>
            </div>
