            ...
            <tbody>
              {assumptions.map(ass => (
                <tr
                  ...
                  className={`border-t border-vscode-border cursor-pointer transition-colors ${selectedId === ass.id ? 'bg-vscode-accent/20' : 'hover:bg-vscode-bg-hover'}`}
                >
                  <td className="p-3">
                    <span className={`w-3 h-3 rounded-full inline-block ${ass.active ? 'bg-green-500' : 'bg-vscode-text-secondary'}`} title={ass.active ? 'Active' : 'Inactive'}></span>
                  </td>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex items-center pt-5">
                <input id="assActive" type="checkbox" checked={editorState.active} onChange={e => handleUpdate('active', e.target.checked)} className="h-4 w-4 rounded border-vscode-border bg-vscode-bg-input text-indigo-600 focus:ring-vscode-accent disabled:cursor-not-allowed" disabled={isReadOnly} />
                <label htmlFor="assActive" className="ml-3 block text-sm font-medium text-vscode-text-primary">Active</label>
              </div>
            </div>
            <div>
