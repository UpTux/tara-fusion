    ...
    <div className="flex h-full text-white">
      ...
      <div className="w-1/3 border-r border-gray-700/50 flex flex-col">
        <div className="p-4 border-b border-gray-700/50 flex justify-between items-center">
          ...
          <button onClick={handleAdd} title="Add new security control" className="p-1.5 text-gray-400 hover:text-white hover:bg-gray-700 rounded disabled:opacity-50 disabled:cursor-not-allowed" disabled={isReadOnly}>
            ...
            <tbody>
              {securityControls.map(sc => (
                <tr
                  ...
                  className={`border-t border-gray-700/50 cursor-pointer transition-colors ${selectedId === sc.id ? 'bg-indigo-600/20' : 'hover:bg-gray-800/50'}`}
                >
                  <td className="p-3 text-center">
                    <span className={`w-3 h-3 rounded-full inline-block ${sc.active ? 'bg-green-500' : 'bg-gray-500'}`} title={sc.active ? 'Active' : 'Inactive'}></span>
                  </td>
                  <td className="p-3 text-center">
                    <span className={`w-3 h-3 rounded-full inline-block ${sc.activeRRA ? 'bg-blue-500' : 'bg-gray-500'}`} title={sc.activeRRA ? 'Active for RRA' : 'Inactive for RRA'}></span>
                  </td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex items-center space-x-6 pt-5">
                    <div className="flex items-center">
                        <input id="scActive" type="checkbox" checked={editorState.active} onChange={e => handleUpdate('active', e.target.checked)} className="h-4 w-4 rounded border-gray-600 bg-gray-700 text-indigo-600 focus:ring-indigo-500 disabled:cursor-not-allowed" disabled={isReadOnly} />
                        <label htmlFor="scActive" className="ml-3 block text-sm font-medium text-gray-300">Active (TARA)</label>
                    </div>
                    <div className="flex items-center">
                        <input id="scActiveRRA" type="checkbox" checked={editorState.activeRRA} onChange={e => handleUpdate('activeRRA', e.target.checked)} className="h-4 w-4 rounded border-gray-600 bg-gray-700 text-indigo-600 focus:ring-indigo-500 disabled:cursor-not-allowed" disabled={isReadOnly} />
                        <label htmlFor="scActiveRRA" className="ml-3 block text-sm font-medium text-gray-300">Active (RRA)</label>
                    </div>
                </div>
            </div>
