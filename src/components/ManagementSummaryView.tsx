...
export const ManagementSummaryView: React.FC<ManagementSummaryViewProps> = ({ project, onProjectChange, isReadOnly }) => {
    ...
    return (
        <div className="p-8 text-white space-y-8 max-w-7xl mx-auto w-full">
            <h2 className="text-3xl font-bold text-gray-200">Management Summary</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700/50">
                    ...
                    ) : <p className="text-center text-gray-500 py-10">No risk data to display.</p>}
                </div>
                <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700/50">
                    <h3 className="text-lg font-semibold mb-4">Risk Treatment</h3>
                    {riskTreatmentData.length > 0 ? (
                        <PieChart data={riskTreatmentData} />
                    ) : <p className="text-center text-gray-500 py-10">No treatment decisions made.</p>}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700/50">
                    <h3 className="text-lg font-semibold mb-4">Top 5 Risk Scenarios</h3>
                    {topRisks.length > 0 ? (
                            ...
                            {topRisks.map(ts => (
                                <li key={ts.id} className="flex items-start space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-400 font-mono">{ts.id}</p>
                                    </div>
                                </li>
                            ))}
                        </ul>
                    ) : <p className="text-center text-gray-500 py-10">No risks to display.</p>}
                </div>
                <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700/50 flex flex-col">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-semibold">Analyst Summary</h3>
                        <button
                    ...
                    <textarea
                        ...
                        className="w-full flex-1 p-3 bg-gray-900/50 border border-gray-600 rounded-md text-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition resize-none"
                        readOnly={isReadOnly}
                    />
                </div>
            </div>
