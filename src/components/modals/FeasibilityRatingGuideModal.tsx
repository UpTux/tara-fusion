...
const FeasibilitySection: React.FC<{ title: string; description: string; data: any[]; columns: { header: string, key: string, className?: string }[]; }> = ({ title, description, data, columns }) => (
  <div className="mb-8">
    <h3 className="text-2xl font-bold text-vscode-text-primary text-indigo-300 mb-1">{title}</h3>
    <p className="text-sm text-vscode-text-secondary mb-3 leading-relaxed">{description}</p>
    <div className="overflow-x-auto">
      <table className="w-full text-left text-sm border-collapse">
        <thead className="bg-vscode-bg-sidebar">
          <tr>
        ...
        <tbody>
          {data.map((row, index) => (
            <tr key={index} className="border-b border-vscode-border">
              {columns.map(col => (
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-vscode-bg-sidebar border border-vscode-border rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex justify-end space-x-4 p-6 border-t border-vscode-border bg-vscode-bg-sidebar">
          <button
            onClick={onClose}
